...
import { ArrowLeft, Check, Gauge, Lightbulb, SlidersHorizontal } from "@lucide/vue";
import { computed } from "vue";
import type { OptimizerConfig, OptimizerKind, TrainingSettings } from "../types";
...
const definitions: Array<{
...
}> = [
  {
    ...
    recommendation: {
      learningRate: 0.018,
      optimizer: {},
      parameters: [{ label: "学习率", value: "0.018" }],
...
const selected = computed(
  () => definitions.find((item) => item.kind === props.settings.optimizer.kind) ?? definitions[0],
);

const recommendationApplied = computed(() => {
  const recommendation = selected.value.recommendation;
  if (props.settings.learningRate !== recommendation.learningRate) return false;
  return Object.entries(recommendation.optimizer).every(
    ([key, value]) => props.settings.optimizer[key as keyof OptimizerConfig] === value,
  );
});
...
function selectOptimizer(kind: OptimizerKind) {
  ...
  updateSettings({ optimizer: { ...props.settings.optimizer, kind } });
}
...
function applyRecommendation() {
  ...
  emit("update", {
    ...props.settings,
    learningRate: recommendation.learningRate,
    optimizer: {
      ...props.settings.optimizer,
...
function normalizeLearningRate(input: HTMLInputElement) {
  const value = Number(input.value);
  if (!Number.isFinite(value) || value <= 0) input.value = String(props.settings.learningRate);
}
...
function setOptimizerParameter(
  ...
  const valid = key === "epsilon"
    ...
    : key === "momentum"
      ...
      : Number.isFinite(value) && value > 0 && value < 1;
  if (!valid) {
    input.value = String(props.settings.optimizer[key]);
    return;
  }
  updateSettings({
    optimizer: { ...props.settings.optimizer, [key]: value },
  });
}
</script>
    ...
    <section class="optimizer-recommendation" aria-label="当前优化器建议参数" aria-live="polite">
      <div class="optimizer-recommendation-copy">
        ...
        <div>
          ...
          <strong>{{ selected.label }} 建议参数</strong>
          <p>{{ selected.recommendation.note }}</p>
        </div>
      </div>
      <div class="optimizer-recommendation-values">
        <span v-for="parameter in selected.recommendation.parameters" :key="parameter.label">
      ...
      >
        <Check :size="15" />
        {{ recommendationApplied ? "已应用" : "应用建议" }}
      </button>
    </section>
    ...
    <section class="optimizer-config-band" aria-labelledby="optimizer-config-heading">
      <header>
        <div>
          ...
          <h2 id="optimizer-config-heading">{{ selected.label }} 参数</h2>
        </div>
        ...
        <label class="optimizer-field">
          ...
          <span class="optimizer-input-shell">
            ...
            <input
              ...
              :disabled="locked"
              data-testid="optimizer-learning-rate"
        ...
        <label v-if="settings.optimizer.kind === 'momentum'" class="optimizer-field">
          ...
          <span class="optimizer-input-shell">
            <input
              ...
              :disabled="locked"
              data-testid="optimizer-momentum"
